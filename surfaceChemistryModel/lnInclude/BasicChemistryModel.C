...
:
    ...
    mesh_(thermo.p().mesh()),
    chemistryVol_(get<Switch> ("chemistryVolume" )),
    chemistrySurf_(get<Switch>("chemistrySurface")),
...
{	
    IOdictionary catalyticSampleDict
    (
        IOobject
        (
	    "catalyticSampleProperties",
            mesh().time().constant(),
            mesh(),
            IOobject::MUST_READ_IF_MODIFIED,
            IOobject::NO_WRITE
    ...
    catalyticLength_ = readScalar(catalyticSampleDict.lookup("Lsample"));
}
